...
void *measure_stride_acesses_mt(void *cookie);
...
void error_print(void) {
    ...
    printf("  > Use \"-m <NUM OF THREADS>\" when using multi thread execution.\n");
}
...
void *measure_stride_acesses_mt(void *cookie) {
    ...
    if (use_threadset) {
        CPU_ZERO(&cpu_set);
        if (state->cpu >= 0) {
            CPU_SET(state->cpu, &cpu_set);
        }
        else {
            cpu_set_all(&cpu_set, cpus_to_set);
        }
        ts_result = sched_setaffinity(tid, sizeof(cpu_set_t), &cpu_set);
        if (ts_result != 0) {
...
loads(size_t max_work_size, size_t size, size_t stride)
{
    ...
    if (!use_multi_thread) {
        ...
        for (int sinit = 1; sinit < num_threads; sinit++) {
            ...
            if (use_threadset) {
                if (sinit >= thread_cpu_num) {
                    state[sinit].cpu = -1;
                }
                else {
                    state[sinit].cpu = cpus_to_set[sinit];
                }
            }
        }
        ...
        for (register int tc = 0; tc < num_threads; tc++) {
            pthread_create(&tsk[tc], NULL, measure_stride_acesses_mt, &state[tc]);
...
void cg_taskset(pid_t pid, char* cpu_to_set){
    ...
    // Create sub group
    create_directory("/sys/fs/cgroup/sim_lmb/inf-sim-lat_mem_rd");

    // Set CPU num 1 to cgroup config
    cg_fp = fopen("/sys/fs/cgroup/sim_lmb/inf-sim-lat_mem_rd/cpuset.cpus", "w");
    if (!cg_fp) {
        printf("ERR: Could not open the croup-v2 cpuset file. (%s)\n", "/sys/fs/cgroup/sim_lmb/inf-sim-lat_mem_rd/cpuset.cpus");
        exit(1);
    }
    ...
    cg_fp = fopen("/sys/fs/cgroup/sim_lmb/inf-sim-lat_mem_rd/cgroup.procs", "w");
    if (!cg_fp) {
        printf("ERR: Could not open the croup-v2 cpuset file. (\"inf-sim-lat_mem_rd\" group)\n");
        exit(1);
    }
    fprintf(cg_fp, "%d\n", pid);
    fclose(cg_fp);
    ...
    do {
        cg_fp = fopen("/sys/fs/cgroup/sim_lmb/inf-sim-lat_mem_rd/cgroup.procs", "r");
        if (!cg_fp) {
            printf("ERR: Could not open the croup-v2 cpuset file. (\"inf-sim-lat_mem_rd\" group)\n");
            exit(1);
        }
...
int main(int argc, char* argv[]) {
    ...
    for (int awc = 0; awc < argc; awc+=2) {       
        switch (argv[awc+3][1]) {
            case 'r':
                use_report = true;
                num_iters = strtoull(argv[(awc+3)+1], &endptr, 10);
                break;
            case 'g':
                use_cg_taskset = true;
            ...
            case 'f':
                ...
                file_name = argv[(awc+3)+1];
                sprintf(log_file_path, "%s%s", "./inf-results/", file_name);
                break;
            case 'e':
                    buf_log_size = strtoull(argv[(awc+3)+1], &endptr, 10);
                    use_buffsize_set = true;
                break;
            case 'm':
                    num_threads = strtoull(argv[(awc+3)+1], &endptr, 10);
                    if (num_threads > 1) 
    ...
    if (use_cg_taskset && use_threadset) {
    ...
    if (num_threads > MAX_MEM_PARALLELISM) {
        printf("Too many threads. (MAX_MEM_PARALLELISM: %d)\n", MAX_MEM_PARALLELISM);
        exit(1);
    }
    ...
    if (use_threadset) {
        ...
        if (thread_cpu_num < num_threads) {
            printf("WARN: The number of threads exceeds the number of thread-set CPUs. (CPU num: %d)\n", thread_cpu_num);
        }
    }
